...
class ScriptSelectActivity : AppCompatActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (id) {
            ...
            R.id.action_open_github -> {
                intent = Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/oror/ara-android"))
                startActivity(intent)
            }
            R.id.action_help -> {
    ...
    companion object {
        ...
        fun getUpdateMessage(lastVersion: Int, version: Int): String? {
            ...
            msg.add(34, "<h3>아라봇 18.1006</h3> Messenger봇 관련된 것들을 모두 제거 <s>광고좆까^^</s>, 레이아웃 및 메뉴를 대폭 개편하였습니다. GPLv3 라이센스로 배포됩니다.")
            msg.add(35, "<h3>아라봇 18.1007</h3> UI, 색, 아이콘 디자인을 대폭 수정하였습니다. (Material Design 대응)")
            msg.add(36, "<h3>아라봇 18.1110</h3> 어색한 번역을 모두 수정하고 영어로된 String을 번역하였습니다. 일본어 번역은 지움 ㅋ")
            msg.add(37, "<h3>아라봇 18.1223</h3> <s>개같은</s>UI 여백과 색을 수정하였습니다.")
            msg.add(38, "<h3>아라봇 18.1224</h3>** 18/12/24 메신저봇의 소스가 내려갔습니다. 시발 Violet 개새키^ㅗ^ **")
            msg.add(39, "<h3>아라봇 19.0127</h3>Utils.getJsonTextByPOST() 함수와 Utils.decodeUnicode()함수, Utils.getAppVersion()함수를 추가했습니다. 자세한 내용은 도움말을 참고하세요.<br>스크립트 삭제시 .js를 입력하지 않으면 삭제되지 않는 사항을 수정하였습니다. (#2)<br>스크립트 삭제시 대소문자를 구별하지 않도록 하였습니다.")
        ...
        fun noti(ctx: Context) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                noti.setChannelId("kr.oror.arabot.notification")
                // Create the NotificationChannel
                val name = ctx.getString(R.string.channel_name)
                val description = ctx.getString(R.string.channel_description)
                val importance = NotificationManager.IMPORTANCE_LOW
                val mChannel = NotificationChannel("kr.oror.arabot.notification", name, importance)
